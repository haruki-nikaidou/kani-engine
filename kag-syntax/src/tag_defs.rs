//! Compile-time validation rules for all known KAG tags.
//!
//! Every tag handled by either the interpreter (`kag-interpreter`) or the
//! runtime bridge (`kani-runtime`) is listed here with its parameter
//! requirements.  The lowering pass calls [`validate_tag`] for every
//! [`Tag`] it encounters and collects the resulting [`ParseDiagnostic`]s
//! alongside the normal parse errors.
//!
//! # Severity policy
//!
//! | Severity | When |
...
fn validate(tag: &Tag<'_>, diags: &mut Vec<ParseDiagnostic>) {
    match tag.name.as_ref() {
        ...
        "chara_hide" | "chara_free" | "chara_mod" => {
            recommend_any_of(tag, &["name", "id"], diags);
        }

        // ── Everything else (runtime extensions, macros, etc.) ─────────────
        //
        // Unknown tags are forwarded to the host as generic events; we have
        // no schema for them and emit no diagnostics.
        _ => {}
    }
}
